...
if 1 == 1:
    ...
    async def quotecmd(message):    # noqa: C901
        ...
        resp = requests.post(f'{config["api_url"]}/api/v2/quote', data=request)
        ...
        if resp["status"] == 500:
            ...
            if resp["message"] != "ERROR_TEMPLATE_NOT_FOUND":
                raise ValueError("Invalid response from server", resp)
            newreq = requests.post(f'{config["api_url"]}/api/v1/getalltemplates', data={
        ...
        req = requests.get(f'{config["api_url"]}/cdn/{resp["message"]}')
